...
void main() {
  ...
  test('equal-second live transition applies without a confirmation query', () {
    scenario((time, container, relay) {
      relay.results.removeAt(0).complete([
        _event('relay', 'online', subject: 'alice', timestamp: 20),
      ]);
      time.flushMicrotasks();
      // A transition in the observation second is at-or-after it: settle
      // last-arrival-wins like an equal-second live heartbeat.
      relay.emit(_event('alice', 'offline', timestamp: 20));
      expect(container.read(presenceCacheProvider)['alice'], 'offline');
      time.elapse(Duration.zero);
